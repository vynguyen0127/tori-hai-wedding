 ...
 *   I  dietary_notes     Free text
 *   J  plus_one_allowed        TRUE | FALSE
 *   K  plus_one_name           Filled in when RSVP is submitted
 *   L  plus_one_dietary_notes  Free text
 *   M  rsvp_submitted_at       ISO timestamp
 */
...
const COL = {
  ...
  PLUS_ONE_ALLOWED:       9,
  PLUS_ONE_NAME:          10,
  PLUS_ONE_DIETARY_NOTES: 11,
  RSVP_SUBMITTED_AT:      12,
};

// Column letters for write operations (A=0 … M=12)
...
export async function getAllGuests(forceRefresh = false) {
  ...
  const res = await sheets.spreadsheets.values.get({
    ...
    range: `${SHEET_NAME}!A2:M`, // Row 1 is headers
  });
 ...
 *   Array of { guestId, status, dietaryNotes, plusOneName, plusOneDietaryNotes }
 */
export async function submitRsvp(responses) {
  ...
  for (const rsvp of responses) {
    ...
    const updates = [
      ...
      { col: COL.PLUS_ONE_DIETARY_NOTES, value: rsvp.plusOneDietaryNotes ?? ''        },
 ...
 * @property {string}  plusOneDietaryNotes
...
function rowToGuest(row) {
  ...
  return {
    ...
    plusOneDietaryNotes:   row[COL.PLUS_ONE_DIETARY_NOTES]  ?? '',
